...
### [NEW] Table 1: Distribution of assessment types.
...
### Table 2: Mental disorders and the studies targeting them: NA = app name not available/not mentioned.
...
To complement the previous table some percentages are discussed in the section 'Assessment types in research studies', as part of the Discussion,  which are included here for reference. 
...
### [NEW] Alternative to TABLE 2. 

_NOTE_: In response to the comment **G.26** 'Characteristics of included studies'. It adds psychological framework data per study to the table 2. In production (manual formating), the table can get more compact, all "NO" can be deleted (empty cell instead), and "YES" can be substituted by coloring the cell depending on the psychological approach, as in the example I provided.

```{r tab1bis, echo=FALSE}
data_kp_apps <- 
    all_data %>%
    select(md_desc, app_name, id, fmw_cbt, fmw_wav, fmw_tra, fmw_pos) %>%
...
data_kp_apps %>%
    ...
    add_header_above(c(" ", "References by app" = 2, "Psychological framework" = 4)) %>%
    column_spec(1, bold = T) %>%
...
A new, separate table 'Breakdown of treatment modalities'. If so, it will most likely go into the Supplementary materials.
...
data_all_apps <- 
  all_data %>%
  select(app_name, md_desc, year, id, fmw_cbt, fmw_wav, fmw_pos, fmw_tra, fmw_other, fmw_nk) %>%
...
sketch <- 
  ...
  thead(
    tr(
      ...
      th(rowspan = 2, 'YEAR'),
      th(rowspan = 2, 'ID'),
      th(colspan = 6, 'PSY FRAMEWORK')
      # th(colspan = 3, 'UX')
    ),
    tr(
      lapply(c('CBT', 'WAV', 'TRA', 'POS', 'OTHER', 'NotK'), th)
