...
# FIXME: tempoeal solcution. Create all_data.rda/csv instead
all_data <- 
  papers2018 %>% 
...
data_tbl_assesstype %>%
    ...
    knitr::kable(format="html", escape = T, booktabs = TRUE,    
          caption = paste0("Distribution of assessment type. ", 
                            "'NA' is Not Applicable/No Data")) %>%
...
# FIXME: Order of mental disorders is not ensured 
# data_plot_barchart$md_desc <- factor(data_plot_barchart$md_desc, levels=md_all_ordered)
...
It responds to the issue [Characteristics of included studies/app](https://github.com/cgranell/apps-treatment-mental-disorders/issues/1]
...
# FIXME: all studies vs studies with appname
# FIXME: same app in different years
...
data_all_apps %>%
  ...
  datatable(container = sketch, rownames = FALSE) %>%
    ...
    formatStyle('TRA',
      backgroundColor = styleEqual(c("YES", "NO"), c('green', ''))) %>%
    formatStyle('POS',
